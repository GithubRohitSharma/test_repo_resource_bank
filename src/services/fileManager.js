...
const { createFileInfo, deleteFileInfo, getFileVotesStatus } = require("../models/rating")
...
class FileManager {
  ...
  async getFiles(parentID, userId) {
    ...
    response.data.files.sort((a, b) => {
      ...
      if (diffA === diffB) return a.name.localeCompare(b.name);
      return diffB - diffA;
    });
  ...
  async deleteFile(fileId) {
    await this.authorize();
    const service = google.drive("v3");
    await service.files.delete({ auth: this.jwtClient, fileId: fileId });
  ...
  async getIDByName(parentID, fileName) {
    ...
    const response = await service.files.list({
      auth: this.jwtClient,
      pageSize: 900,
      q: `'${parentID}' in parents and name = '${safeName}'`,
      fields: 'files(id, name)'
    });
    const files = response.data.files || [];
    if (files.length === 0) {
      throw new Error(`No file or folder named '${fileName}' found under parent '${parentID}'`);
    }
    return files[0].id;
  }
  ...
  async getScheduleOrFaculty(type) {
    ...
    const service = google.drive('v3');
    let parentID = '';
    if (type == 'schedule') parentID = this.SCHEDULE;
    else if (type == 'faculty') parentID = this.FACULTYID;
  ...
  async updateFaculty(fileId, body, file) {
    ...
    try {
        ...
        await service.files.get({ auth: this.jwtClient, fileId, fields: 'id, name, properties' });
